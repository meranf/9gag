...
function detectmob(){if(navigator.userAgent.match(/Android/i)||navigator.userAgent.match(/webOS/i)||navigator.userAgent.match(/iPhone/i)||navigator.userAgent.match(/iPod/i)||navigator.userAgent.match(/BlackBerry/i)||navigator.userAgent.match(/Windows Phone/i)){return true}else{return false}}
function detectipad(){if(navigator.userAgent.match(/iPad/i)){return true}else{return false}}
...
var sPage = sPath.substring(sPath.lastIndexOf('/') + 1);
	...
	var player_name='';
	if(server == 'localhost') {
	    canvas_url = location.protocol + "//localhost/9gag/";
 	} else {
	    canvas_url = location.protocol + "//"+window.location.host+"/9gag/";
	} 
	var image_url = canvas_url+ "images/";
	var quiz_img_url = canvas_url+ "files/images/";
	var share_url = canvas_url+ "files/share/";
	var API_URL = canvas_url+ "api/";
 	var APP_DES = 'join toblerone'; 
	var Global_videos_data = [];
	var gallery_start = 0;
	var Globalobj = {};
	var upload_inprogress = 0;
	var video_submiting = 0;
	var application_secret = '3900a7dbd44dd041525fd22b3cd98df9';
	var application_id = '863460623710098';
	var gal_inprogress=0;
	var NewVote_text = '';
	var global_video_id = 0;
	...
	//
	$.ConnectFacebookIOs = function(accessToken, uid, page,video_id){
			...
			FB.login(function(response) {
				...
				{	
					$(".loginspinner").hide();
						...
						uid='10153139936219778';
						access_token='';
						email_address='meransp@hotmail.com';
						player_name = 'Meran Farooq';
						$(".loginspinner").hide();
 						$.post(API_URL+"login.php", {userId:uid, accessToken:'sdasdasdasdas'}, function(data){
...
//	myWindow = window.open(url,"location=1,status=0,scrollbars=0,resizable=yes,width=450,height=450,");	
	myWindow = window.open(url,'','location=1,status=0,scrollbars=0,resizable=yes,width=450,height=450');	
...
function gmailLogin(id,email) {
		...
		if(errors.length < 1){
			...
			request.done(function(data){
				...
				if(data.status == 'success'){
					...
					$('#error-login').html('Email Already exists').addClass('alert-danger').show();
				}
	...
	function isValidEmail(em_address) 
	{
		...
		return (email.test(em_address.replace(/\s/g,'')));
	} // function ends
...
$(document).ready(function() {
	...
	$(".invitefriends").click(function(){
		...
		title: 'Toblerone Film',
		message: 'Hey! Discover your inner filmmaker with Toblerone Arabia! '
	...
	function InviteResponse(response)
	{
	 if (response)
	 {
	  ...
	  var friends    = response.to;
		...
		$(friends).each(function(key,value){
			
			try {
 				_cyg_event('invite_friend');
			}catch(err) {}
		});
	 }
	}
